...
class image_utils():
  ...
  def generate_words(self,image_name, score_bbox, image, crop_dir):
      ...
      if bbox_coords != ['{}']:
        ...
        if np.all(pts) > 0:
          ...
          try:
            ...
            print(
              'Image saved to ' + file_name + '_{}_{}_{}_{}_{}_{}_{}_{}.jpg'.format(l_t, t_l, r_t, t_r, r_b, b_r, l_b,
                                                                                    b_l))
          except:
            continue

  def crop_bound_images(self,csv_path, test_folder, crop_dir):
    # Create the crop dir
    ...
    for image_num in range(data.shape[0]):
      image = cv2.imread(str(os.path.join(start, data['image_name'][image_num])))
      image_name = data['image_name'][image_num].strip('.jpg')
      score_bbox = data['word_bboxes'][image_num].split('),')
      self.generate_words(image_name, score_bbox, image, crop_dir)
